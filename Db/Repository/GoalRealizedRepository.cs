...
namespace BeforeIDie.Db.Repository
{
    public class GoalRealizedRepository : IGoalRepository
    {
        ...
        {
            return await _db.GoalItems.Where(g => g.Status == Status.Realized).FirstOrDefaultAsync(g => g.Id == id);
        }
        ...
        {
            var fromDb = await _db.GoalItems.Where(g => g.Status == Status.Realized).FirstOrDefaultAsync(g => g.Id == id);
            if (fromDb == null) { return false; }
